...
nonempty_expr_list: nonempty_expr_list ',' nonempty_expr { 
                    $$ = append_Expressions($1, single_Expressions($3)); 
                  }
                  | nonempty_expr { $$ = single_Expressions($1); }
                  | { $$ = nil_Expressions(); }

nonempty_block: nonempty_expr ';' nonempty_block {
                  $$ = append_Expressions(single_Expressions($1), $3); 
                }
                | nonempty_expr ';' { $$ = single_Expressions($1); }
                | error {}

sub_let: OBJECTID ':' TYPEID ASSIGN expr IN expr {
          $$ = let($1, $3, $5, $7);
        }
        | OBJECTID ':' TYPEID IN expr {
          $$ = let($1, $3, no_expr(), $5);
        }
        | OBJECTID ':' TYPEID ASSIGN expr ',' sub_let {
          $$ = let($1, $3, $5, $7);
        } 
        | OBJECTID ':' TYPEID ',' sub_let {
          $$ = let($1, $3, no_expr(), $5);
        }

nonempty_case_list: case ';' nonempty_case_list {
...
while_exp : WHILE nonempty_expr LOOP expr POOL { $$ = loop($2, $4); }
          | WHILE nonempty_expr LOOP error {}

expr: nonempty_expr { $$ = $1; }
      | { $$ = no_expr(); }

nonempty_expr: OBJECTID ASSIGN nonempty_expr { $$ = assign($1, $3); }
              | nonempty_expr '.' OBJECTID  '(' nonempty_expr_list ')' { $$ = dispatch($1, $3, $5); }
              | nonempty_expr '@' TYPEID '.' OBJECTID  '(' nonempty_expr_list ')' { 
                $$ = static_dispatch($1, $3, $5, $7); 
              }
              | OBJECTID '(' nonempty_expr_list ')' { 
              ...
              | LET sub_let { $$ = $2; }
              | CASE nonempty_expr OF nonempty_case_list ESAC { $$ = typcase($2, $4); }
              | NEW TYPEID { $$ = new_($2); }
