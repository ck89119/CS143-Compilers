...
int labelnum = 0;
CgenClassTable* codegen_class_table = nullptr;
...
void program_class::cgen(ostream &os) {
  ...
  CgenClassTable *codegen_class_table = new CgenClassTable(classes, os);
  // codegen_class_table->code();
...
static void emit_jal(const char *address, ostream &s) { s << JAL << address << endl; }
...
void StringEntry::code_def(ostream& s, int stringclasstag) {
  ...
  s << WORD; s << Str << DISPTAB_SUFFIX; s << endl;                                              // dispatch table
...
void IntEntry::code_def(ostream &s, int intclasstag) {
 ...
 /***** Add dispatch information for class Int ******/
  s << WORD; s << Int << DISPTAB_SUFFIX; s << endl;                                              // dispatch table
...
void BoolConst::code_def(ostream& s, int boolclasstag)
{
 ...
 /***** Add dispatch information for class Bool ******/
  s << WORD; s << Bool << DISPTAB_SUFFIX; s << endl;                                              // dispatch table
...
CgenClassTable::CgenClassTable(Classes classes, ostream &s): nds(NULL), str(s) {
  ...
  InitNameTagMap();
  stringclasstag = name_tag_map[Str];
  intclasstag = name_tag_map[Int];
  boolclasstag = name_tag_map[Bool];
...
void CgenClassTable::InitNameTagMap() {
  if (class_nodes.empty()) {
    ...
    for (int i = 0; i < class_nodes.size(); ++i) {
      ...
      name_tag_map[class_nodes[i]->get_name()] = i;
    }
  }
}
...
void CgenClassTable::code_class_name_tab() {
  ...
  for (CgenNode* class_node : class_nodes) {
    ...
    for (List<CgenNode>* l = class_node->get_children(); l; l = l->tl()) {
      str << "# child: " << l->hd()->name << endl;
    }
    str << std::endl;
  }
}
...
void CgenClassTable::code_dispatch_tabs() {
  for (CgenNode* _class_node : class_nodes) {
    ...
    for (method_class* _method : full_methods) {
      ...
      str << WORD; emit_method_ref(_class_name, _method_name, str); str << endl;
    }
  }
}
...
void static_dispatch_class::code(ostream &s, Environment env) {
  ...
  CgenNode *_class_node = codegen_class_table->GetClassNode(type_name);
...
void dispatch_class::code(ostream &s, Environment env) {
  ...
  CgenNode *_class_node = codegen_class_table->GetClassNode(_class_name);
  s << "\t# Now we locate the method in the dispatch table." << endl;
...
void typcase_class::code(ostream &s, Environment env) {
  std::map<Symbol, int> _class_tags = codegen_class_table->GetNameTagMap();
  std::vector<CgenNode *> _class_nodes = codegen_class_table->GetClassNodes();
  ...
  auto GetChildrenTagsSet = [&](std::vector<int> __curr_tags) {
    ...
    for (int __curr_tag : __curr_tags) {  // find children of this class.
      ...
      for (List<CgenNode>* l = __curr_node->get_children(); l; l = l->tl()) {
        CgenNode *__children_node = l->hd();
